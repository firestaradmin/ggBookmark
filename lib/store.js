...
(function () {
  ...
  // 版本号以 manifest.json 为唯一权威来源，运行时读取，避免多处维护不同步。
  // manifest.json 的 version 是 Chrome 扩展必需的，本文件无需再单独改动版本号。
  GG.VERSION = (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.getManifest && chrome.runtime.getManifest().version) || '0.0.0';

  GG.DEFAULTS = {
    backgroundImage: '',
    backgroundStyle: 'default', // default | gradient | preset | image | color
    backgroundBlur: 0,          // px applied to the bg image layer
    backgroundDim: 0.15,        // 0..1 dark overlay opacity on top of image
    cardCols: 5,                // 卡片固定列数（不随页面宽度变化）
    cardMinWidth: 280,          // 卡片最小宽度 (px)，页面放不下时出现横向滚动条
    glassBlur: 20,              // 磨砂玻璃模糊度 (px)，顶部工具栏/卡片/置顶行
    glassOpacity: 0.06,         // 磨砂玻璃背景透明度 (0~1)
    glassColor: '#ffeec2',      // 磨砂玻璃背景主色（用户可选任意颜色）
    searchEngine: 'bing',
    fontSize: 'medium',
    accentColor: '#4f7cff',
    ...
    openInPlace: false, // true = 左键单击书签/置顶项直接在当前标签页跳转，不再另开新标签；Ctrl/Cmd+左键仍在后台打开新标签
    sync: {
      enabled: false,
      server: '',
      username: '',
      ...
      direction: 'both',     // both（双向，默认）| up（仅上传）| down（仅下载）
      syncBookmarks: true,   // 是否同步书签内容（false 时仅同步设置与卡片配置）
      versionedBackup: true, // 版本化备份：默认开启，每次上传额外保存带时间戳的快照（当天保留最新 3 个，历史每天保留最后 1 个、共保留最近 5 份）
      triggers: [],          // subset of: interval | settingsChange | bookmarkChange | startup
      intervalMinutes: 30
    }
  };
