...
(function () {
  ...
  // 返回按优先级排列的候选图标地址列表：首选用户配置的来源，其后依次尝试其它
  // 在线来源作为兜底（避免单一服务 404 / 超时导致图标缺失），local 除外。
  GG.faviconChain = function (url, source) {
    const host = faviconHost(url);
    if (!host) return [];
    source = source || (_root.__ggSettings && _root.__ggSettings.faviconSource) || GG.DEFAULTS.faviconSource;
    if (source === 'local') return [];
    const order = [source, 'faviconsnap', 'duckduckgo', 'google'].filter((s, i, arr) => s && arr.indexOf(s) === i);
    return order.map((s) => faviconFor(host, s)).filter(Boolean);
  };
