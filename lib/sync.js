...
(function () {
  ...
  async function scheduleAlarm() {
    ...
    console.log('[gg-sync] scheduleAlarm:', { enabled: cfg.enabled, triggers, interval: cfg.intervalMinutes });
    if (cfg.enabled && triggers.includes('interval') && Number(cfg.intervalMinutes) > 0) {
      ...
      console.log('[gg-sync] alarm created. current alarms:', all);
    } else {
      console.log('[gg-sync] alarm NOT created (enabled/triggers/interval invalid)');
    }
  }
  ...
  async function handleAlarm(name) {
    ...
    const cfg = await getSyncCfg();
    const triggers = Array.isArray(cfg.triggers) ? cfg.triggers : [];
    console.log('[gg-sync] alarm fired. enabled=' + cfg.enabled + ' triggers=' + JSON.stringify(triggers));
    if (!cfg.enabled || !triggers.includes('interval')) {
      browser.runtime.sendMessage({ type: 'gg-sync-log', ok: false, msg: '定时同步被跳过：未启用或定时触发未勾选' }).catch(() => {});
      return;
    }
    try {
      await upload();
  ...
  if (typeof browser !== 'undefined' && browser.alarms) {
    ...
    if (browser.runtime && browser.runtime.onStartup) {
      browser.runtime.onStartup.addListener(() => { scheduleAlarm().catch(() => {}); });
    }
    if (browser.runtime && browser.runtime.onInstalled) {
      browser.runtime.onInstalled.addListener(() => { scheduleAlarm().catch(() => {}); });
    }
  }
