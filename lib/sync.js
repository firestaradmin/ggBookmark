...
(function () {
  ...
  // 浏览器启动时多条同步触发源（onStartup / handleBookmarkChange / 页面 maybeSync）几乎同时
  ...
  // ---------- 设置变更触发同步（后台） ----------
  // 页面（newtab）已通过 storage.onChanged 触发 settingsChange，但设置页单独打开时
  // 没有该监听。后台统一监听 storage，避免“只开设置页就不同步”的盲区。
