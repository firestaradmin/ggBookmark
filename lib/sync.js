...
(function () {
  ...
  async function localHasChanged(meta) {
    ...
    if (cfg.syncBookmarks === false) {
      // 不同步书签时，用 lastLocalChangeAt 与 lastSyncAt 的先后判断
      return (meta.lastLocalChangeAt || 0) > (meta.lastSyncAt || 0);
    }
    try {
  ...
  async function doDownload(cfg) {
    ...
    const metaBefore = await getLocalMeta();
    ...
    await patchSettingsMeta({
      ...
      lastLocalChangeAt: metaBefore.lastLocalChangeAt || 0
    });
    await setSyncState({
      lastSuccessAt: now,
  ...
  async function smartSync(opts) {
    ...
    if (state.status === 'remote-newer') {
      ...
      if (info && info.diff && info.diff.big && opts.resolve !== 'remote') {
        const conflict = {
          type: 'big-change',
          time: Date.now(),
  ...
  async function handleBookmarkChange() {
    try {
      // 记录本地变更时间点，供冲突判定使用
      await patchSettingsMeta({ lastLocalChangeAt: Date.now() });
      const cfg = await getSyncCfg();
      const triggers = Array.isArray(cfg.triggers) ? cfg.triggers : [];
