 ...
 *  - 同步下载会把远端当作当前同步源做“合并”：当本地与上次同步基线完全一致时，
 *    删除本地多余的书签/文件夹，使删除操作也能从一台机器传播到另一台；
 *    手动导入、版本恢复和没有同步基线的首次下载仍保持“只增不删”，避免误删。
 */
(function () {
  'use strict';
  ...
  // 全局互斥：保证同一时间只有一个「下载并重建书签」（recreateBookmarks）在执行。
  // 浏览器启动时多条同步触发源（onStartup / handleBookmarkChange / 定时 alarm）几乎同时
  // 命中，若都判为 remote-newer 而并发 doDownload，会并发重建书签树。
  // 而 recreateBookmarks 只增不删、文件夹按 title 去重而书签无并发保护，并发时会把
  // 新文件夹里的书签 create 两次（文件夹复用不重复、书签重复一份）。
  let rebuildChain = Promise.resolve();
  // 全局同步互斥：串行化所有 smartSync 调用，避免多个页面 / 后台同时触发同步时并发
  // 上传/下载互相覆盖（一个页面上传后，另一个页面用稍旧的本地快照又上传覆盖远端）。
  // 多个触发源会在同一时刻都调用 smartSync，这里把它们排成队列依次执行。
  let syncChain = Promise.resolve();
  // 全局同步节流：短时间内的重复触发合并为一次，避免 N 个打开的新标签页各触发一次上传。
  // 距离上次真正执行同步不足该毫秒数时，跳过重复触发（已有一次在途/刚完成即可）。
  const SYNC_COALESCE_MS = 5000;
  let lastSyncRunAt = 0;
  // 版本化备份保留策略：
  //  - 当天备份保留最新 VERSIONS_KEEP_PER_DAY 个
  //  - 历史每天保留该天最后一次上传的 1 个快照，共保留最近 VERSIONS_KEEP_DAYS 份（不论相隔几天）
  //  合计最多 VERSIONS_KEEP_PER_DAY + VERSIONS_KEEP_DAYS 个。
  const VERSIONS_KEEP_PER_DAY = 3;   // 当天保留的版本份数
  const VERSIONS_KEEP_DAYS = 5;      // 历史每天保留最后 1 个，共保留的每日快照份数
  ...
  // 远端配置期望的书签/文件夹集合（按路径 + 标题 + URL 定位），供同步合并判断删除项。
  function collectRemoteSets(trees) {
    ...
    function walk(nodes, prefix) {
      (nodes || []).forEach((c) => {
        ...
        if (isFolder) {
          const p = prefix.concat(c.title || '');
          folderPaths.add(p.join(PATH_SEP));
          walk(c.children || [], p);
        } else if (c.url) {
          bookmarkKeys.add(prefix.concat(JSON.stringify([c.title || c.url, c.url])).join(PATH_SEP));
        }
      });
    }
    (trees || []).forEach((sub) => {
      if (sub && sub.node) walk(sub.node.children || [], []);
    });
    return { folderPaths, bookmarkKeys };
  }
  ...
  async function recreateBookmarks(trees, targetParentId) {
    // 通过全局互斥串行化，避免并发重建书签树时把同一书签 create 两次。
    ...
    // 无论成功与否都让链条继续推进，避免一次失败卡死后续同步
    rebuildChain = run.catch(() => {});
    return run;
  }
  // 同步合并：仅当远端是当前同步源时，删除本地在远端配置中不存在的书签/文件夹。
  // 与 recreateBookmarks 共用重建互斥，避免与并发下载相互穿插。
  async function reconcileBookmarks(trees, targetParentId) {
    const run = rebuildChain.then(async () => {
      const remote = collectRemoteSets(trees);
      const root = targetParentId || await resolveToolbarId();
      const childrenCache = new Map();
      async function getChildren(parentId) {
        if (!childrenCache.has(parentId)) {
      ...
      const removals = [];
      async function collect(parentId, prefix) {
        const kids = await getChildren(parentId);
        for (const k of kids) {
          const isFolder = k.type ? k.type === 'folder' : !k.url;
          if (isFolder) {
            const p = prefix.concat(k.title || '');
            const pathKey = p.join(PATH_SEP);
            if (!remote.folderPaths.has(pathKey)) {
              removals.push({ id: k.id, tree: true, depth: p.length });
            } else {
              await collect(k.id, p);
            }
          } else if (k.url) {
            const key = prefix.concat(JSON.stringify([k.title || k.url, k.url])).join(PATH_SEP);
            if (!remote.bookmarkKeys.has(key)) removals.push({ id: k.id, tree: false });
          }
        }
      }
      await collect(root, []);
      removals.sort((a, b) => ((b.tree ? b.depth : 0) - (a.tree ? a.depth : 0)));
      for (const r of removals) {
        if (r.tree) await GG.api.bookmarks.removeTree(r.id);
        else await GG.api.bookmarks.remove(r.id);
      }
  ...
  async function applyConfig(imported, opts) {
    opts = opts || {};
    ...
    if (imported.bookmarks) {
      ...
      let remapped;
      try {
        // 同步下载时以远端为准：先删除本地多余项，再补齐远端缺失项。
        // 该模式只由 doDownload 在“本地未改过书签（与基线一致）”时开启，
        // 手动导入 / 版本恢复 / 首次无基线下载不会走到这里，保持只增不删。
        if (opts.syncReconcile) {
          await reconcileBookmarks(imported.bookmarks, target);
        }
  ...
  function normalizeTriggers(sync) {
    ...
    triggers = triggers.filter((t) => ['interval', 'settingsChange', 'bookmarkChange', 'cardChange', 'startup'].includes(t));
    s.triggers = triggers;
    if (!['both', 'up', 'down'].includes(s.direction)) s.direction = 'both';
    // 版本化备份已默认开启且不再提供开关，这里强制为 true，
    // 避免旧数据中保存的 false 覆盖默认值（保证所有用户都启用）
    s.versionedBackup = true;
    delete s.mode;
    return s;
  }
  ...
  // ---------- 版本化备份 ----------
  // 版本文件名：在基准文件名后追加时间戳与书签数量，
  // 如 ggbookmark-config.20260823-1158-173104.json（1158 为书签数量）。
  // 书签数量写入文件名，可让「查看版本备份」直接读取名称即可展示，无需下载解析文件。
  ...
  function versionFilename(cfg, date, bookmarkCount) {
    const parts = splitFilename(cfg.filename);
    const count = (bookmarkCount != null && !isNaN(Number(bookmarkCount)))
      ? '-' + Number(bookmarkCount)
      : '';
    return parts.base + '.' + stampOf(date) + count + parts.ext;
  }
  function isVersionFilename(cfg, name) {
    const parts = splitFilename(cfg.filename);
    const esc = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    // 时间戳固定 14 位（YYYYMMDD-HHMMSS），书签数量为可选的纯数字段（可能为空）
    const re = new RegExp('^' + esc(parts.base) + '\\.(\\d{8}-\\d{6})(?:-(\\d+))?' + esc(parts.ext) + '$');
    ...
    return {
      stamp: s,
      time: time || 0,
      bookmarkCount: m[2] != null && m[2] !== '' ? Number(m[2]) : null
    };
  }
  ...
  // 上传版本快照并清理旧版本。
  // 保留策略（versions 已按时间倒序）：当天（最新日期）保留最新 VERSIONS_KEEP_PER_DAY 个；
  // 历史每天只保留该天最后上传的 1 个快照，共保留最近 VERSIONS_KEEP_DAYS 份（不论相隔几天）。
  async function uploadVersionSnapshot(cfg, body, bookmarkCount) {
    const name = versionFilename(cfg, new Date(), bookmarkCount);
    const base = (cfg.server || '').trim().replace(/\/+$/, '');
    const url = base + '/' + name;
    await putRemote(url, body, cfg);
    try {
      const versions = await listVersions(cfg);
      // 按日期（stamp 前 8 位 YYYYMMDD）分组；versions 已按时间倒序，组内也是新→旧
      const byDay = new Map();
      for (const v of versions) {
        const day = v.stamp ? v.stamp.slice(0, 8) : String(v.time || 0);
        if (!byDay.has(day)) byDay.set(day, []);
        byDay.get(day).push(v);
      }
      // 日期按该天最新版本时间倒序（最近日期在前）
      const days = [...byDay.keys()].sort((a, b) => byDay.get(b)[0].time - byDay.get(a)[0].time);
      const keep = new Set();
      days.forEach((day, idx) => {
        const list = byDay.get(day); // 该天内新→旧
        if (idx === 0) {
          // 今天（最新日期）：保留最新 VERSIONS_KEEP_PER_DAY 个
          list.slice(0, VERSIONS_KEEP_PER_DAY).forEach((v) => keep.add(v.name));
        } else if (idx <= VERSIONS_KEEP_DAYS) {
          // 历史每天：只保留该天最后 1 个快照，共保留最近 VERSIONS_KEEP_DAYS 份
          keep.add(list[0].name);
        }
      });
      const extras = versions.filter((v) => !keep.has(v.name));
  ...
  // 归一化 ETag 的表示差异（强/弱 ETag 前缀、成对引号），避免同一内容因 PUT/HEAD/GET
  // 返回的 ETag 表示形式不同而被误判为「远端已变化」，导致假冲突。
  ...
  async function assessState(cfg) {
    ...
    // 允许 2s 时钟误差
    const SKEW = 2000;
    // ETag 相同说明远端内容与上次同步时完全一致：即使服务器刷新了 Last-Modified
    // （或 HEAD/GET 时间戳不一致），也不应视为远端有变化。
    // 但 ETag 不同不能只看 Last-Modified：HTTP 时间是秒级精度，同一秒内两次上传
    // 的 Last-Modified 可能完全相同，仅靠时间戳会漏掉真正的远端变更。
    const hasEtagPair = !!(meta.lastRemoteETag && remote.etag);
    const remoteChanged = hasEtagPair
      ? normalizeETag(meta.lastRemoteETag) !== normalizeETag(remote.etag)
      : (!syncedTs || !remoteTs || (remoteTs - syncedTs > SKEW));
  ...
  async function doUpload(cfg) {
    ...
    const putRes = await putRemote(resolveUrl(cfg), body, cfg);
    // PUT 响应头通常带有服务器侧 Last-Modified / ETag。优先用它们作为新的同步基线，
    // 避免上传后 probe 失败时回退到本地时钟，产生时钟偏差造成的“假远端变更”。
    const putLastModified = putRes.headers.get('Last-Modified');
    const putETag = putRes.headers.get('ETag') || null;
    const putTs = putLastModified ? (new Date(putLastModified).getTime() || null) : null;
    // 版本化备份：当天保留最新 3 个，历史每天保留最后 1 个、共保留最近 5 份（不论相隔几天）
    let versionInfo = null;
    if (cfg.versionedBackup) {
      try { versionInfo = await uploadVersionSnapshot(cfg, body, data.bookmarkCount); } catch (e) { /* 版本备份失败不阻塞主上传 */ }
    }
    // 上传成功后记录远端时间戳与本地同步点
    let remoteMeta = null;
    ...
    await patchSettingsMeta({
      ...
      // 时间基线同样优先用上传后的 HEAD 探测（probeRemote）值，与 assessState 的
      // 比较来源保持一致（它拿 probeRemote().lastModified 与 lastRemoteModified 比较）。
      // 若改用 PUT 响应的 Last-Modified，而服务器 PUT/HEAD 返回的时间不同（>2s 时钟误差），
      // 会在服务器不返回 ETag（走时间比较）时把「刚上传过的远端」永久误判为已变化，
      // 导致每次设置更改都报「与远端冲突」。仅当上传后探测失败时才回退到 PUT / 本地时间。
      lastRemoteModified: (remoteMeta && remoteMeta.lastModified) || putTs || (data.exportedAt || now),
      // 用 HEAD 探测（probeRemote）返回的 ETag 作为同步基线：assessState 判定远端
      // 是否变化时也是拿 probeRemote().etag 与 lastRemoteETag 比较。若改用 PUT 响应的
      // ETag，部分 WebDAV 服务器 PUT 与 HEAD 返回的 ETag 表示形式不同（强/弱、引号、
      // 算法差异），会永久误判为「远端已变化」，与本地改动叠加后产生假冲突。
      lastRemoteETag: (remoteMeta && remoteMeta.etag) || putETag || '',
  ...
  async function doDownload(cfg) {
    ...
    if (!cfg.server) throw new Error('未配置同步');
    const meta = await getLocalMeta();
    // 仅当存在同步基线且本地书签仍与基线一致时，才允许按远端精确删除本地书签。
    // 这保证“远端是当前同步源、删除确实发生在同步源侧”，避免把无关远端配置或
    // 第一次接入（无基线）误当作删除指令；手动下载 / 版本恢复也会传入 false。
    ...
    // 允许删除的条件是“本地与上次同步基线一致”（本机没改过书签），而不是远端指纹
    // 等于基线：远端恰恰是因为删除了书签，指纹才会变化。此时远端才是当前同步源，
    // 本地多余的书签应被移除；本地已改过（conflict）时不会自动走到这里。
    let localFingerprint = '';
    try {
      localFingerprint = (await buildConfig({ includeBookmarks: true })).bookmarkFingerprint || '';
    } catch (e) { /* ignore */ }
    const allowReconcile = !!(meta.lastSyncAt && meta.lastSyncedFingerprint && localFingerprint && localFingerprint === meta.lastSyncedFingerprint);
    // 先保留本地同步元数据（applyConfig 会整体覆盖 settings），应用后再修补回来
    const applied = await applyConfig(imported, { syncReconcile: allowReconcile });
    const now = Date.now();
    ...
    await patchSettingsMeta({
      ...
      lastRemoteModified: res.lastModified || (imported.exportedAt || now),
      lastRemoteETag: res.etag || '',
  ...
  // 也通过 syncChain 串行化，避免与后台自动同步并发执行互相覆盖。
  function upload() {
    const run = syncChain.then(() => doUpload());
    syncChain = run.catch(() => {});
    return run;
  }
  function download() {
    const run = syncChain.then(() => doDownload());
    syncChain = run.catch(() => {});
    return run;
  }
  ...
  async function _smartSyncImpl(opts) {
    opts = opts || {};
    const cfg = await getSyncCfg();
    ...
    if (state.status === 'remote-newer') {
      // 首次同步（本机从未同步过）：若本地几乎没有书签，直接下载远端配置并跳过
      // “大幅覆盖”确认；若本地已有真实书签且远端覆盖会造成大幅变化，仍需用户确认，
      // 避免把远端旧/不同来源的配置静默当作唯一真源。
      ...
      if (neverSynced) {
        ...
        if (info && info.diff && info.diff.big && opts.resolve !== 'remote' && !localIsBare) {
          ...
          return { action: 'big-change', info: conflict, firstDownload: true };
        }
        const applied = await doDownload(cfg);
      ...
      let info = null;
      try { info = await buildConflictInfo(state.remoteMeta); } catch (e) { /* ignore */ }
      // 正常同步的远端更新通常只包含另一台设备的少量增删。若远端相对本地差异超过
      // 20%，才进入“大幅覆盖”确认；这里已用本地指纹做过删除保护，少量删除可安全下载。
      const deletionOnly = info && info.diff && info.diff.removed > 0 && info.diff.added === 0;
      const localHasSyncBaseline = !!(state.localMeta.lastSyncAt && state.localMeta.lastSyncedFingerprint);
      // 本地几乎没有任何书签（新电脑 / 尚未导入真实数据）时，同样不因“大幅覆盖”而阻断下载，
      // 否则首次接入时外观/壁纸/设置等会因 >20% 变化被跳过，无法一并恢复。
      const localIsBare = info && info.diff && (info.diff.localCount || 0) <= 1;
      const safeReconcile = localHasSyncBaseline && deletionOnly;
      if (info && info.diff && info.diff.big && opts.resolve !== 'remote' && !localIsBare && !safeReconcile) {
        const conflict = {
          type: 'big-change',
          time: Date.now(),
          remoteLastModified: info.remoteLastModified,
  ...
  // 智能同步的对外入口：用全局互斥串行化 + 节流合并。
  // - 互斥：同一时间只执行一个同步，避免多页面/后台并发上传互相覆盖。
  // - 节流：非手动触发（resolve 为空）且距上次实际同步不足 SYNC_COALESCE_MS 时直接跳过，
  //   减少 N 个新标签页同时触发导致的重复同步。
  // 手动同步（opts.resolve 有值或 opts.force 为 true）不节流，保证用户点「立即同步」一定执行。
  async function smartSync(opts) {
    opts = opts || {};
    const run = syncChain.then(async () => {
      const now = Date.now();
      const isManual = !!opts.resolve || !!opts.force;
      if (!isManual && now - lastSyncRunAt < SYNC_COALESCE_MS) {
        await setSyncState({ lastCheckAt: now, lastError: '', conflict: null });
        return { action: 'in-sync', coalesced: true };
      }
      const r = await _smartSyncImpl(opts);
  ...
  // ---------- 设置变更触发同步（后台） ----------
  // 设置变更由后台统一监听触发：页面（newtab）的 storage.onChanged 在多个标签页中
  // 每个都会触发各自的防抖同步（节流是各上下文独立的），会造成多标签页重复上传；
  // 而后台是唯一的入口，天然合并所有标签页的设置变更，且不依赖页面是否打开。
  // 同步引擎自己写入的元数据字段不视为用户设置变更。
  const SETTINGS_META_KEYS = ['lastSyncAt', 'lastLocalChangeAt', 'lastRemoteModified', 'lastRemoteETag', 'lastSyncedFingerprint', 'configVersion'];
  function settingsSubstantiallyChanged(prev, next) {
    ...
    for (const k of keys) {
      if (SETTINGS_META_KEYS.includes(k)) continue;
      const a = prev[k];
      const b = next[k];
      if (k === 'sync') {
  ...
  async function handleSettingsChange(changes, area) {
    ...
    try {
      ...
      if (!settingsSubstantiallyChanged(prev, next)) return;
      const cfg = await getSyncCfg();
      const triggers = Array.isArray(cfg.triggers) ? cfg.triggers : [];
      if (!cfg.enabled || !triggers.includes('settingsChange')) return;
      if (settingsSyncTimer) clearTimeout(settingsSyncTimer);
      settingsSyncTimer = setTimeout(async () => {
        settingsSyncTimer = null;
        await autoSync('设置更改').catch(() => {});
      }, 30000); // 与页面防抖一致，合并连续保存
    } catch (e) { /* 忽略配置读取错误 */ }
  }

  // ---------- 卡片变更触发同步（后台） ----------
  // 卡片数据存于独立 storage 键（apps / categories / orderByCat / activeCat / pinned），
  // 由后台统一监听触发「卡片更改」同步：页面（newtab）在多个标签页中每个都会各自触发
  // 防抖同步（节流上下文独立），会造成多标签页重复上传；后台是唯一入口，天然合并。
  const CARD_CHANGE_KEYS = ['apps', 'categories', 'orderByCat', 'activeCat', 'pinned'];
  let cardSyncTimer = null;
  async function handleCardChange(changes, area) {
    if (area !== 'local' || !changes) return;
    if (!CARD_CHANGE_KEYS.some((k) => k in changes)) return;
    try {
      if (await isRestoring()) return;
      // 记录本地变更时间点，供冲突判定使用；确保严格大于上次同步点，避免同毫秒误判
      const meta = await getLocalMeta();
      ...
      const cfg = await getSyncCfg();
      const triggers = Array.isArray(cfg.triggers) ? cfg.triggers : [];
      if (!cfg.enabled || !triggers.includes('cardChange')) return;
      if (cardSyncTimer) clearTimeout(cardSyncTimer);
      cardSyncTimer = setTimeout(async () => {
        cardSyncTimer = null;
        await autoSync('卡片更改').catch(() => {});
      }, 30000); // 防抖合并连续操作（拖动卡片、连点保存）
    } catch (e) { /* 忽略配置读取错误 */ }
  }
  ...
  if (isBackground && GG.api.alarms && GG.api.alarms.onAlarm) {
    ...
    if (GG.api.storage.onChanged) {
      GG.api.storage.onChanged.addListener(handleSettingsChange);
      GG.api.storage.onChanged.addListener(handleCardChange);
    }
  }
