...
(function () {
  ...
  async function assessState(cfg) {
    ...
    // （或 HEAD/GET 时间戳��一致），也不应视为远端有变化。
    const etagUnchanged = !!(meta.lastRemoteETag && remote.etag && meta.lastRemoteETag === remote.etag);
    const remoteChanged = !etagUnchanged && (!syncedTs || !remoteTs || (remoteTs - syncedTs > SKEW));
