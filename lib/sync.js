...
(function () {
  ...
  //  - 历史每天保留该天最后一次上传的 1 个快照，共保留最近 VERSIONS_KEEP_DAYS 天
  //  合计最多 VERSIONS_KEEP_PER_DAY + VERSIONS_KEEP_DAYS 个。
  const VERSIONS_KEEP_PER_DAY = 3;   // 当天保留的版本份数
  const VERSIONS_KEEP_DAYS = 5;      // 历史保留的天数（每天保留最后 1 个）
  ...
  // 历史每天只保留该天最后上传的 1 个快照，共保留最近 VERSIONS_KEEP_DAYS 天。
    ...
    try {
      ...
      days.forEach((day, idx) => {
        ...
        if (idx === 0) {
          ...
          // 历史每天：只保留该天最后 1 个快照，共最近 VERSIONS_KEEP_DAYS 天
  ...
  async function doUpload(cfg) {
    ...
    // 版本化备份：当天保留最新 3 个，历史每天保留最后 1 个、共 5 天
