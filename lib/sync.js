...
(function () {
  ...
  async function handleBookmarkChange() {
    try {
      ...
      }, 30000); // 书签变更后延迟 再上传，避免频繁操作刷屏
    } catch (e) { /* 忽略配置读取错误 */ }
  }

  // ---------- 浏览器启动触发同步（后台） ----------
  // 浏览器启动时若启用「startup」触发，则自动上传一次。
  async function handleStartup() {
    try {
      ...
      const filename = cfg.filename || 'ggbookmark-config.json';
      const base = { source: '浏览器启动', target: 'webdav', filename };
      try {
        await upload();
        await log(Object.assign({}, base, { type: 'upload', ok: true, msg: '浏览器启动自动上传成功' }));
      } catch (e) {
        await log(Object.assign({}, base, { type: 'upload', ok: false, msg: '浏览器启动自动上传失败：' + (e && e.message) }));
      }
    } catch (e) { /* 忽略配置读取错误 */ }
  }
